...
void onBoard_LCD_I2C_init(void){ //Initializes all components of SLCD on the FRDM-KL46Z
...
void onBoard_LCD_I2C_Set(uint8_t Value,uint8_t Digit){//Sets a value from 0-F to a specified Digit, with 1 being the leftmost, 4 being the rightmost.  Will not display error is Value is outside of 0-F, but display will not update
	//int k;
	if(Digit > 4){
		onBoard_LCD_I2C_DisplayError(0x01);
...
}//End onBoard_LCD_I2C_Set

void onBoard_LCD_I2C_DisplayDecimal(uint16_t Value){//Displays a 4 Digit number in decimal
	if(Value > 9999){
		onBoard_LCD_I2C_DisplayError(0x10); //Display "Err" if value is greater than 4 digits
	}
	else{
		onBoard_LCD_I2C_Set(Value/1000,1);
		onBoard_LCD_I2C_Set((Value - (Value/1000)*1000)/100,2);
		onBoard_LCD_I2C_Set((Value - (Value/100)*100)/10,3);
		onBoard_LCD_I2C_Set(Value - (Value/10)*10,4);
	}
}//End SegLCD_Display4Digit

void onBoard_LCD_I2C_DisplayHex(uint16_t Value){ //Displays a 16 bit number in Hex Format
	onBoard_LCD_I2C_Set((Value & 0xF000)>>12, 1);
	onBoard_LCD_I2C_Set((Value & 0x0F00)>>8 , 2);
	onBoard_LCD_I2C_Set((Value & 0x00F0)>>4 , 3);
	onBoard_LCD_I2C_Set((Value & 0x000F)>>0 , 4);
}//End SegLCD_DisplayHex

void onBoard_LCD_I2C_DisplayTime(uint8_t Value1, uint8_t Value2){//Displays 2 values separated by a colon
	if((Value1 > 99) | (Value2 > 99)){
		onBoard_LCD_I2C_DisplayError(0x10); //Display "Err" if either value is greater than 2 digits
	}
	else{
		onBoard_LCD_I2C_Set(Value1/10, 1);
		onBoard_LCD_I2C_Set(Value1 % 10, 2);
		onBoard_LCD_I2C_Set(Value2/10, 3);
		onBoard_LCD_I2C_Set(Value2 % 10, 4);
		SegLCD_Col_On();
	}
}//End SegLCD_DisplayTime

void onBoard_LCD_I2C_DisplayError(uint8_t ErrorNum){//Displays Err# on screen, where # is a value 0-F.  If ErrorNum is outside of that range, just displays Err
	...
	if(ErrorNum < 0x10){
		onBoard_LCD_I2C_Set(ErrorNum,4); //Display ErrorNum in digit 4 if within valid range.  If not, leave blank.
	}
	else{
		LCD->WF8B[LCD_FRONTPLANE6] = (LCD_CLEAR);
		LCD->WF8B[LCD_FRONTPLANE7] = (LCD_CLEAR);
	}
}//End onBoard_LCD_I2C_DisplayError
